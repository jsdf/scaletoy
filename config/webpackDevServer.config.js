...
module.exports = function(proxy, allowedHost) {
  return {
    ...
    before(app, server) {
      ...
      app.get(/wam/, function(req, res, next) {
        const resolvedFilepath = path.resolve('./public/' + req.path);
        const mimetype = getMimeType(resolvedFilepath);
        res.type(mimetype);
        console.log('served wam path', req.path, mimetype);
        res.send(fs.readFileSync(resolvedFilepath));
      });
